...
class CalcService {
    ...
    fun calc(expression: String): String {
        ...
        //   5+5/3+7*9
        //   (5+5)/3+7*9
        var result: String = expression.replace(Patterns.SPACES.toRegex(), "")
    ...
    private fun calcOneOperation(
        ...
        return String.format(Locale.ENGLISH, "%4.5f", result)
    }

    private fun calcBracedExpression(expressionWithBraces: String): String {
        var expression = expressionWithBraces
        while (expression.contains(Patterns.BRACES.toRegex())) {
            var braceCounter = 0
            val openBraceIndex = expression.indexOf(Patterns.BRACES)
            ...
            val tempExpression = if (bracedExpression.contains(Patterns.BRACES.toRegex())) {
